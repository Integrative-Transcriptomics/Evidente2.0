...
class NodeInformation extends Component {
    state = { showSupport: false, showNonSupport: false, showParaphyletic: false };
                        ...
                        <Accordion>
                            ...
                            <Card>
                                <Accordion.Toggle
                                    as={Card.Header}
                                    style={{ backgroundColor: "lightblue" }}
                                    eventKey={this.state.showNonSupport ? "2" : "-1"}
                                    id='paraphyleticSNPs-header'
                                >
                                    Paraphyletic SNPs
                                </Accordion.Toggle>
                                <Accordion.Collapse eventKey='2' id='paraphyleticSNPs-card'>
                                    <Card.Body style={{ maxHeight: 250, overflow: "auto" }}>
                                        <VirtualizedTable
                                            rows={get(this.props.SNPTable, `paraphyletic.actualNode`, [])}
                                            title={"SNPs in all leaves below the node"}
                                            noSNPtitle={"No SNPs in all leaves below the node"}
                                            onMultipleSNPaddition={this.props.onMultipleSNPaddition}
                                            onSNPaddition={this.props.onSNPaddition}
                                        />
                                        <VirtualizedTable
                                            rows={get(this.props.SNPTable, `paraphyletic.descendants`, [])}
                                            title={"SNPs in subset of leaves below the node"}
                                            noSNPtitle={"No SNPs in any subset of leaves below the node"}
                                            onMultipleSNPaddition={this.props.onMultipleSNPaddition}
                                            onSNPaddition={this.props.onSNPaddition}
